...
from cimmyt.op.context import ContextLocal
from cimmyt.utils.decorators import try_or_none, try_or_dict
from cimmyt.utils.filetools import import_object, gzip_decode
from cimmyt.utils.logtools import trace
from cimmyt.utils.regex import rx_template_name
from cimmyt.utils.serializers import jsondumps, jsonloads
from cimmyt.utils.strtools import camel_case_split
from cimmyt import compat, constants
from cimmyt.errors import ApiRequestError, ApiResponseError
...
class BaseHandler(web.RequestHandler):
    ...
    @is_finished
    def json_response(self, chunk):
        self.set_header('Content-Type', 'application/json; charset=UTF-8')
        self.finish(self.json_sanitizer(chunk))
            ...
            if isinstance(chunk, Exception):
                ...
                for func in constants.PRIMITIVE_METHODS:
                    if hasattr(chunk, func):
                        ...
                        break
            if not normalize:
                return self.json_response(chunk)
            self.normalize_response(code, message, chunk)
        except Exception, e:
