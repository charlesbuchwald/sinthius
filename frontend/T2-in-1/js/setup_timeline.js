...
// ------------------------ setup container and slides position-----------------------------
	...
	$('menuHor').eq(index).children('.horLink').first().addClass('active');
	 ...
	 $('.horLink').click(function(e){
		...
		scrollContainer.children('slider').css('transition', 'all 500ms ease-in-out').css('margin-left', margin);
...
$('.slideIn').not('#'+that+'In').removeClass('out');
$('#'+that+'In').addClass('out');
 console.log(that+'clicked'+that+ 'In opened');
return false;
})
